...
test("uses a recent stay before asking the model", async () => {
...
test("leaves a cluster unnamed when the model is not sure", async () => {
  const named = await nameRunningPaths(
    [
    ...
    {
      knownAreas: [],
      lookupName: async (center) => (center[0] > 48 ? "Paris" : null),
    }
  );
